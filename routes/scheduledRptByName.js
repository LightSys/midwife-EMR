...
var doReport = function(flds, writable) {
  var options = {
        ...
        , size: 'letter'
        , info: {
            Title: 'Scheduled Report by Name'
            , Author: 'Midwife-EMR Application'
            , Subject: 'Scheduled Report by Name'
        }
      }
  ...
  getData(opts.fromDate, opts.toDate)
    .then(function(rawData) {
      ...
      // --------------------------------------------------------
      // Sort records by lastname, firstname.
      // --------------------------------------------------------
...
var run = function(req, res) {
  ...
  writable.on('finish', function() {
    fs.stat(filePath, function(err, stats) {
      ...
      var size = stats.size;
      var downloadFilename = generateReportFilename('ScheduledRptByName', 'pdf');
      ...
      switch (reportFormat) {
        case FORMAT_SCREEN:
          res.setHeader('Content-Disposition', 'inline; ScheduledRptByName.pdf');
